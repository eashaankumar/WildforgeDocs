...
A fixed-size unmanaged UTF-8 string occupying **128 bytes** of memory.

`UnmanagedString128` stores its contents inline without allocating managed memory, making it suitable for `TypedEntityPools`, unmanaged collections, Burst jobs, and native memory.
...
```csharp
public unsafe struct UnmanagedString128 :
    IEquatable<UnmanagedString128>
```
...
Stores up to **127 UTF-8 bytes**.

The remaining byte stores the string length.

ASCII text can contain up to **127 characters**.
